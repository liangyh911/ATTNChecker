...
// template<typename T> __device__ T* chk_v_a;
// template<typename T> __device__ T* chk_v_b;
  ...
  if constexpr (std::is_same<T, float>::value) {
    if (m == 72 && k == 64){
      abftbgemm<float, 72, 72, 64>(transa, transb, m, n, k,
        ...
        num_batches,
        COL_FT,ROW_FT,DEBUG,CHECK_BEFORE,CHECK_AFTER, ifPassChk);
    }
    else if(m == 64 && k == 72){
...
void myGemm(char transa, char transb, int64_t m, int64_t n, int64_t k, at::opmath_type<T> alpha,
  ...
  // std::ifstream myfile("/home/exouser/control/IFPassChk.txt");
  // if (myfile.is_open()){
  //   char flag;
  //   myfile.get(flag);
  //   myfile.close();
...
// template <typename T> 
// __device__ T* colchk_Gptr, *rowchk_Gptr;

// template <typename T> 
// __device__ T* rowchk_Gptr;
