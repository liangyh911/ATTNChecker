...
void bgemm<float>(CUDABLAS_BGEMM_ARGTYPES(float)) {
  ...
  // fs::path destinationFile = "abftbgemm/records/time/abftbgemm.txt";
  // cudaDeviceSynchronize();
  ...
  // recordTime(fullPath, (duration.count() / 1000.0), false);
}
...
void abftbgemm(char transa, char transb, int64_t m, int64_t n, int64_t k, at::opmath_type<T> alpha,  
  ...
  // destinationFile = "abftbgemm/records/time/preparation.txt";
  ...
  if (COL_FT && CHECK_AFTER) {
    ...
    encode_col_v5<T, M, N, 4><<<num_batches, dim3(M*4, 1), (M+1)*N*sizeof(T), stream_main>>>(num_batches,
                   dC, lddc, stridec, 
                    dC_colchk_r<T>, lddc_colchk_r, (2*n));

    if (DEBUG) cudaEventRecord(start, stream_main);
    T E = 1;
    detect_correct_col<T><<<dim3(num_batches), dim3(n), 0, stream_main>>>(dC, lddc, E, stridec,
    ...
    if (DEBUG) {
      ...
      cudaEventElapsedTime(&t1, start, stop);
      // destinationFile = "abftbgemm/records/effeciency/abftbgemm.txt";
      destinationFile = "abftbgemm/records/time/BGemmCorrect.txt";
      fullPath = homePath / destinationFile;
  ...
  if (ROW_FT && CHECK_AFTER) {
    ...
    encode_row_v5<T, M, N><<<num_batches, dim3(M*2, 1, 1), 0, stream_main>>>(num_batches,
                    dC, lddc, stridec,
                    dC_rowchk_r<T>, lddc_rowchk_r, (2*m));
    T E = 1;
    if (DEBUG) cudaEventRecord(start, stream_main);
    detect_correct_row<T><<<dim3(num_batches), dim3(m), 0, stream_main>>>(dC, lddc, E, stridec, n,
  ...
  if(ifPassChk && QKV == 'v'){    
    ...
    BGemmChkMerge_v3<<<blocks, threads, 0, stream_main>>>(dC_rowchk<T>, m, 2, num_head, 
    ...
    MatrixRowReduceSum<<<blocks1, threads, 0, stream_main>>>(tmp_chk<T>, m*num_head, 2*(num_batches/num_head), (num_batches/num_head),
...
void abftGemmPassChk(char transa, char transb, int64_t m, int64_t n, int64_t k, 
  ...
  if (ROW_FT){
    ...
    if(DEBUG){
      ...
      if(QKV == 'q'){
        destinationFile = "abftbgemm/records/time/Q.txt";
        fullPath = homePath / destinationFile;
        recordTime(fullPath, t1, DEBUG);
      }
      else if(QKV == 'k'){
        destinationFile = "abftbgemm/records/time/K.txt";
        fullPath = homePath / destinationFile;
        recordTime(fullPath, t1, DEBUG);
      }
      else if(QKV == 'v'){
  ...
  if (DEBUG)  {
    ...
    if(COL_FT){
      ...
      destinationFile = "abftbgemm/records/time/V.txt";
      fullPath = homePath / destinationFile;
      recordTime(fullPath, t_Achk, DEBUG);
    ...
    if(ROW_FT && QKV != 'k'){
      ...
      if(QKV == 'q'){
        destinationFile = "abftbgemm/records/time/Q.txt";
        fullPath = homePath / destinationFile;
        recordTime(fullPath, t_Bchk, DEBUG);
      }
      else if(QKV == 'v'){
        destinationFile = "abftbgemm/records/time/V.txt";
        fullPath = homePath / destinationFile;
...
void gemm<float>(CUDABLAS_GEMM_ARGTYPES(float)) {
  ...
  // fs::path destinationFile = "abftbgemm/records/time/abftgemm.txt";
  // fs::path fullPath = homePath / destinationFile;
  // recordTime(fullPath, (duration.count() / 1000.0), false);
}
...
void abftGemmBias(
  ...
  if (ROW_FT){
    ...
    dim3 threads(threadsDim,threadsDim);
    if(DEBUG) cudaEventRecord(start, stream_main);
    GemmMatrxiChkMerge_v3<<<blocks, threads, 0, stream_main>>>(mat2_ptr, k, n, dB_rowchk<T>, 1, B_copy, k, n_copy);
    if(DEBUG){
      cudaEventRecord(stop, stream_main);
      cudaEventSynchronize(stop);
      cudaEventElapsedTime(&t1, start, stop);
      ...
      destinationFile = "abftbgemm/records/time/OUT.txt";
      fullPath = homePath / destinationFile;
      recordTime(fullPath, t1, DEBUG);
    }
    // printf("B_copy: \n");
  ...
  dim3 threads(threadsDim, threadsDim);
  if(DEBUG) cudaEventRecord(start, stream_main);
  GemmBiasRowCopyBack<<<blocks, threads, 0, stream_main>>>(C_copy, m_copy, n_copy, result_ptr, m, n, dC_rowchk<T>, bias);
  if(DEBUG){
    cudaEventRecord(stop, stream_main);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&t1, start, stop);
...
void gemm_and_bias(
  ...
  // fs::path destinationFile = "abftbgemm/records/time/abftBias.txt";
  // fs::path fullPath = homePath / destinationFile;
  // fullPath = homePath / destinationFile;
  // recordTime(fullPath, (duration.count() / 1000.0), false);
}
